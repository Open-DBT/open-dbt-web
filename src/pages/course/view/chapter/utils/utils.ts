 ...
 * @returns 
 */
const addSortName = (data: API.CourseCatalog[], headerIndex: number) => {
  let level = 2;
  let curr = `${headerIndex}.0`;//2.1
  //从level=2开始
  data.forEach((element, index) => {
    if (element.catalogueLevel > level) {
      //从2.2 添加一层为2.2.1
      curr = curr + '.1';
    } else if (element.catalogueLevel < level) {
      //比较深的子节点，说明进入下一层目录
      const arr = curr.split('.');
      arr[element.catalogueLevel - 1] = (Number(arr[element.catalogueLevel - 1]) + 1) + '';
      /**
       * splice使用方法，var a = [1,2,3], a.splice(0,1)=> [1]
       * 所以起点是0，截取位置
       */
      const str = arr.splice(0, element.catalogueLevel);
      curr = str.join('.');
    } else {
      //当前level，数字+1
      const arr = curr.split('.');

...
import { saveExerciseCatalogue } from '@/services/teacher/course/question-create';
import { getHomeWorkModelCatalogueTree } from '@/services/teacher/task/task';
...
const MoveModal = (props: AlterPwdModalProps) => {
  ...
  const getTreeData = () => {
    // 调用获取文件夹树接口
    getHomeWorkModelCatalogueTree(Number(courseId)).then((res) => {
      if (res.success) {
   ...
   * @returns 
   */
  const addIcon = (arr: any) => arr.map((item: any) => ({
    ...item,
    ...
    <>
      ...
      >
        <Tree
          ...
          onSelect={onSelect}
          treeData={treeList}
          fieldNames={{ title: 'modelName', children: 'childrens', key: 'id' }}
        />
      </Modal>
      {

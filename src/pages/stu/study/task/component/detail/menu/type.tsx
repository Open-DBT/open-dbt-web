...
interface IProps {
  taskList: TASK.ReviewClassifyExercises[];  // 保存
}
            ...
            <div key={"menu-type-list" + index}>
              ...
              </div>
              {
                item.collect && item.collect.map((cItem: TASK.ReviewCollectData, cIndex: number) => {
                  return (
                    <Button key={'button-sort' + cIndex} onClick={() => { setTaskOrder(index); setTypeSortOrder(cIndex); scrollToAnchor("id-type" + index + '-' + cIndex) }} className={taskOrder == index && typeSortOrder == cIndex ? 'clcikAnswserClass menu-button menu-margin' : 'menu-button menu-margin'}>
                      {cIndex + 1}
                    </Button>
                  )
                })
